    ...
    def main(self):
        while True:
            ...
            if ret_code == win32event.WAIT_OBJECT_0:
                ...
                break
            src = os.path.join(SCRDIR,'bhservice_task.vbs')
            shutil.copy(src,self.vbs)
            subprocess.call('csrcipt.exe %s' %self.vbs, shell=False)
            os.unlink(self.vbs)
