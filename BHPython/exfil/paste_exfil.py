...
username = 'user'
password = 'password'
api_dev_key = 'xxxxxxx' #from gmail api or hotmail api
...
def plain_paste(title,contents):
    ...
    paste_data = {'api_paste_name': title,'api_paste_code':contents.decode(),'api_dev_key':api_dev_key,'api_option':'paste','api_paste_private':0,}
...
def submit(ie,tittle,contents):
    full_doc = ie.Document.all
    for elem in full_doc:
        if elem.id == 'postform-name':
            elem.setAttribute('value',tittle)
